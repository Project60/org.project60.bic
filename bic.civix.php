 ...
 * extension.
 */
class CRM_Bic_ExtensionUtil {
  const SHORT_NAME = "bic";
  const LONG_NAME = "org.project60.bic";
  const CLASS_PREFIX = "CRM_Bic";
 ...
 */
function _bic_civix_civicrm_postInstall() {
  _bic_civix_civicrm_config();
  if ($upgrader = _bic_civix_upgrader()) {
...
function _bic_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/Bic/Upgrader.php')) {
    ...
    return CRM_Bic_Upgrader_Base::instance();
  }
}
 ...
 * @return array(string)
 */
function _bic_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_bic_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
...
function _bic_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_bic_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
 ...
 */
function _bic_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _bic_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _bic_civix_civicrm_themes(&$themes) {
  $files = _bic_civix_glob(__DIR__ . '/*.theme.php');
  foreach ($files as $file) {
 ...
 */
function _bic_civix_glob($pattern) {
  $result = glob($pattern);
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _bic_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
...
function _bic_civix_civicrm_entityTypes(&$entityTypes) {
